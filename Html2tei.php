...
class Html2tei {
  public static function transform($srcfile, $dstfile) {
    ...
    $doc->loadHTMLFile($srcfile); // PHP 5.4 LIBXML_NOCDATA | LIBXML_NOENT | LIBXML_NONET | LIBXML_NOBLANKS
...
<b>Note 360:</b> 
    */
    ...
    $doc->loadXML($xml);
    $xml=$trans->transformToXML($doc);
    
    if (!file_exists(dirname($dstfile))) mkdir(dirname($dstfile), null, true);
    file_put_contents($dstfile, $xml);
  }

  public static function docli() {
    array_shift($_SERVER['argv']); // shift first arg, the script filepath
    if (!count($_SERVER['argv'])) exit('
      usage    : php -f Html2tei.php src/*.xml dstdir/
    ');
    ...
    foreach( glob( $srcglob) as $srcfile) {
      $dstfile=$dstdir.pathinfo($srcfile, PATHINFO_FILENAME).'.xml';
      echo $srcfile, ' > ', $dstfile, "\n";
      self::transform($srcfile, $dstfile);
    }
  }
}
